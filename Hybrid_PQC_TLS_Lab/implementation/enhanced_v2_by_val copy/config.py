...
class CertificateConfig:
    ...
    def __init__(self, algorithm: str, base_dir: str = None):
        ...
        if base_dir is None:
            # 获取项目根目录（config.py所在位置向上两级）
            config_dir = Path(__file__).parent.parent.parent
            base_dir = str(config_dir / "enhanced_certificates")
        self.base_dir = base_dir
    
    def get_cert_paths(self) -> dict:
        """获取证书文件路径"""
        algo_base = f"{self.base_dir}/{self.algorithm}"
        ...
        return {
            ...
            # 客户端信任存储
            "trust_store_cert": f"{algo_base}/client/trust_store/root_ca.crt",
            "trust_store_sig": f"{algo_base}/client/trust_store/root_ca_pq.sig",
        }
